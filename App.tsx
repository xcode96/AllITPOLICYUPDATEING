...
const App: React.FC = () => {
  ...
  const handleDownloadConstants = () => {
    // Generate the TS file content dynamically
    // Use set to dedup if needed, or just straight dump
    const fileContent = `import { Category } from './types';

export const DATA_CATEGORIES: Category[] = ${JSON.stringify(categories, null, 4)};
`;
    triggerDownload('constants.ts', fileContent, 'text/typescript');
  };
  ...
  return (
    <>
      <DashboardLayout
        ...
        onDownloadSource={handleDownloadConstants}
      >
        <PolicyDetail
